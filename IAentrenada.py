...
def procesar_estrella_desconocida(ruta_archivo):
    ...
    # Eztraemos las caracteristicas de la estrella nueva
    res = minimize_scalar(longitud_cuerda, bounds=(0.01, 1.1), args=(df['HJD'].values, df['Mag'].values), method='bounded')
    P = res.x
    
    # Vector de las 4 variables 
    features = np.array([
        np.log10(P), 
        np.max(df['Mag']) - np.min(df['Mag']), 
        skew(df['Mag']), 
        np.mean(df['Mag'])
    ]).reshape(1, -1)
